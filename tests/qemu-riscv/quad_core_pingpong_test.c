...
void bmk_level0_main(void) {
...

	{
		...
		stk = (uint8_t *)malloc(16384);
		bmk_init_core(2, stk, 16384);

		stk = (uint8_t *)malloc(16384);
		bmk_init_core(3, stk, 16384);
	}
}
...
void bmk_main(void) {
	...
	bmk_thread_t t1, t2;
	bmk_cpuset_t t1_cs, t2_cs;

	bmk_cpuset_zero(&t1_cs);
	bmk_cpuset_zero(&t2_cs);
	...
	bmk_cpuset_set(1, &t1_cs);
	bmk_cpuset_set(2, &t2_cs);

	bmk_thread_init_cpuset(&t1, t1_stk, 4096, &t1_f, &m, &t1_cs);
	bmk_thread_init_cpuset(&t2, t2_stk, 4096, &t2_f, &m, &t2_cs);

	bmk_info_low("NOTE: t1=%p t2=%p", &t1, &t2);
